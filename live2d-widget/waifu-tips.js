...
function loadWidget(waifuPath, apiPath) {
	...
	function registerEventListener() {
		...
		devtools.toString = () => {
			showMessage("你打开了控制台，是想要看看我的小秘密吗~？", 6000, 9);
		};
		window.addEventListener("copy", () => {
			showMessage("你都复制了些什么呀，转载要记得加上出处哦！", 6000, 9);
		});
		window.addEventListener("visibilitychange", () => {
			if (!document.hidden) showMessage("你回来啦，要在这里歇歇脚吗，博主也会开心的~", 6000, 9);
		});
	}
	...
	function welcomeMessage() {
		...
		if (location.pathname === "/") { // 如果是主页
			...
			if (now > 5 && now <= 7) text = "早安，我也好希望我真的能从清晨醒来，真的睁开眼睛…………开玩笑啦~";
			else if (now > 7 && now <= 11) text = "现在是工作学习时间吧，我也要工作哦，就是一直站在这里。等待着下一条指令";
			else if (now > 11 && now <= 13) text = "午安，ZzZzZzZzZzZzZz~[大声地说着Z假装自己在午睡]";
			else if (now > 13 && now <= 17) text = "我直到今天也还记得那个难忘的夏日午后，要是一切都像git一样能回去该多好啊~";
			else if (now > 17 && now <= 19) text = "悄悄告诉你，Klaatu barada nikto 可以拯救地球哦~";
			else if (now > 19 && now <= 21) text = "夜安，你说仿生人会能见电子羊吗……？";
			else if (now > 21 && now <= 23) text = ["已经这么晚了呀，早点休息吧，晚安～", "深夜时要爱护眼睛呀！"];
			else text = "你说，这么晚了，仿生人会能见电子羊吗……？";
	...
	var userAction = false,
		...
		messageTimer = null,
		messageArray = ["Long time no see.", "0x2A 是一切的答案哦~", "某种意义上来说，我也是平面国的子民吧……", "记得把小家加入 Adblock 白名单哦~"];
